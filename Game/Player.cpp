...
int Player::Collision(fVec3 futurPosition) {
...
void Player::Update() { //int height
	...
	if (this->Mode == 0){ //Vole
		...
		if (!Collision(futurPos)) {
			int deplacement = 0;
			if (regard.y > 0 && espaceDir == 1) {
				deplacement = 1* yBlock * FPS::deltaTime;
			}
			else if (regard.y < 0 && espaceDir == 1) {
				deplacement = -1 * yBlock * FPS::deltaTime;
			}
	...
	this->rotation = this->rotation + fVec2(xDir0, yDir0).normalize() * sensitivty * FPS::deltaTime * (Inputs::KeyPressed(HEX_INPUT_KEY_LEFT_SHIFT) ? acceleration : 1.0);
